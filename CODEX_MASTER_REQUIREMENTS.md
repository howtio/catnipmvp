...
1. 先补齐所有层的进度日志文件
2. 再初始化 TypeScript 基础文件
3. 再创建十层 `wrapper.ts / types.ts / index.ts`
4. 再开始 Phase 1 的 Queue 与 Worker 骨架
...
1. `npm run typecheck` 或等价命令
2. `npm test` 或项目定义的测试命令
3. 必要时增加 `npm run build`
...
## 18. 为了支持 GitHub 仓库测试与回滚，用户必须提供的信息
...
## 19. 新 Codex 在连接 GitHub 仓库后必须做的第一件事
