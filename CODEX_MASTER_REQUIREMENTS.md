...
6. 检查 `git status`
7. 明确本轮开发目标
8. 明确本轮收尾需要更新哪些日志、是否需要 push、是否允许回滚
...
- 未经测试通过，不允许上传代码实现
- 未更新日志，不允许上传
- 每次代码开发收尾时，必须显式检查是否要 push
- 如果用户的当前要求明显包含“继续开发并按文档执行”，默认收尾时不得遗漏 push 检查
...
8. “按开发文档进行开发”默认包含：读文档、测试、补日志、检查 push、检查回滚。
9. “继续下一步开发”默认不等于“只改代码”，收尾时仍必须完成日志和 GitHub 检查。
