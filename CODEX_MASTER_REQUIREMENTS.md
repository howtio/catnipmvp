...
6. 检查 `git status`
7. 明确本轮开发目标
8. 明确本轮收尾需要更新哪些日志、默认完成后要 push、如需回滚必须先问用户
...
5. 更新本轮涉及层的进度日志
6. 检查是否需要回滚或局部修复；如需回滚必须先问用户
7. 检查是否需要提交
8. 检查本轮开发完成后是否已 push 到 GitHub；默认必须 push
9. 在最终输出里明确说明测试、日志、push、回滚判断结果
...
- 如果测试失败，必须先判断“修复还是回滚”，但回滚动作前必须先问用户
...
如果后续你提供 GitHub 仓库，本项目默认采用“先开发、再测试、完成即上传、是否回滚先问用户”的协作方式。
