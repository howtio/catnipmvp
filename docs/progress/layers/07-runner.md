...
## 2026-05-19 / Runner / 增加打开链接与更长步数预算

### 当前目标

让 Runner 在搜索之后能继续打开目标链接，并把默认思维链路从过短的预算拉长。

### 本次完成

- heuristic provider 新增 `open_url`
- heuristic provider 支持从任务文本提取显式 URL
- provider prompt 明确搜索后可用 `open_url`
- 默认 `maxSteps` 从 `5` 提升到 `10`
